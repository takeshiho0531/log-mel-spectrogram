...
module spi_if # (
  parameter al = 16, // do not change
  parameter dl = 24 // do not change
)(
  input rst_n_i,
  input spi_cs_i,
  input spi_mosi_i,
  ...
  output reg spi_miso_en_o,

  output [al-2:0] addr_out_o,
  output reg [dl-1:0] wr_data_o,
  ...
  output [4:0] spi_reg_out_req_addr_o  // 14桁なので....
);
  ...
  assign transfer_enable_flag_o = transfer_enable_flag_spi ^ transfer_enable_flag_nnn;
  ...
  always @(posedge spi_clk_i or negedge rst_n_i) begin  // Positive Edge
    if (!rst_n_i) begin
      ...
      transfer_enable_flag_spi <= 0;
      count <= {cl{1'b0}};
    end else if (transfer_enable_flag_o == 1'b0) begin
  ...
  always @(posedge spi_clk_i or negedge rst_n_i) begin  // Positive Edge
    ...
    else if (transfer_enable_flag_o == 1'b0) begin
  ...
  integer out_addr;
  reg [4:0] spi_reg_out_req_addr_tmp;
  assign spi_reg_out_en = rd_data_i[14];
  assign data_out = (spi_reg_out_en && out_addr == 13) ? rd_data_i[dl-1-1:0] : data_out;
  assign spi_reg_out_req_addr_o = spi_reg_out_req_addr_tmp;

  always @(posedge spi_clk_b or negedge rst_n_i) begin  // Negative Edge
    $display("rd_data_i=%b, spi_miso_o=%d, spi_miso_en_o=%d, transfer_enable_flag_o=%d, spi_reg_out_en=%d, out_addr=%d, spi_reg_out_req_o=%d", rd_data_i, spi_miso_o, spi_miso_en_o, transfer_enable_flag_o, spi_reg_out_en, out_addr, spi_reg_out_req_o);
    if (!rst_n_i) begin
      // data_out <= {dl{1'b0}};
      spi_miso_o <= 1'b0;
      spi_miso_en_o <= 1'b0;  // For miso bus
      out_addr <= 13;
      spi_reg_out_req_o <= 0;
      spi_reg_out_req_addr_tmp <= 0;
    end else if (transfer_enable_flag_o == 0) begin
      if (spi_reg_out_en && out_addr==13) begin
        spi_miso_o <= data_out[13];
        spi_miso_en_o <= 1;
        out_addr <= 12;
        spi_reg_out_req_addr_tmp <= spi_reg_out_req_addr_tmp;
        spi_reg_out_req_o <= 0;
      end if (spi_reg_out_en && out_addr > 0) begin
        spi_miso_o <= data_out[out_addr];
        spi_miso_en_o <= 1;
        out_addr <= out_addr - 1;
        spi_reg_out_req_addr_tmp <= spi_reg_out_req_addr_tmp;
        spi_reg_out_req_o <= 0;
      end else if (spi_reg_out_en && out_addr == 0) begin
        spi_miso_o <= data_out[0];
        spi_miso_en_o <= 1;
        out_addr <= 13;
        spi_reg_out_req_addr_tmp <= spi_reg_out_req_addr_tmp + 1;
        spi_reg_out_req_o <= 1;
      end else if (out_addr==13) begin // when starting
        spi_miso_o <= 0;
        spi_miso_en_o <= 0;
        out_addr <= 13;
        spi_reg_out_req_addr_tmp <= 0;
